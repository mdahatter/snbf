...
<aui:form action="<%= configurationURL %>" method="post" name="fm1">
	...
	<aui:fieldset label="lists">
		...
		>
			...
			List<DDLRecordSet> lista = DDLRecordSetLocalServiceUtil.search(company.getCompanyId(), scopeGroupId, keywords, DDLRecordSetConstants.SCOPE_DYNAMIC_DATA_LISTS, searchContainer.getStart(), searchContainer.getEnd(), searchContainer.getOrderByComparator());
			List<DDLRecordSet> newLista = new ArrayList<DDLRecordSet>();
			
			CharSequence cs1 = "DDL";
			CharSequence cs2 = "Formulario1"; 
			
			for (int i = 0; i < lista.size(); i++){
				if(lista.get(i).getName().contains(cs1) || lista.get(i).getName().contains(cs2))
				{
